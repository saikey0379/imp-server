...
import (
	...
	shandler "github.com/saikey0379/imp-server/pkg/server/handler"
)
...
func BatchPowerOn(ctx context.Context, w rest.ResponseWriter, r *rest.Request) {
	...
	for _, info := range infos {
		...
		status, err := shandler.GetDevicePowerStatusFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if err != nil {
		...
		runErr := shandler.PowerOnDeviceFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if runErr != nil {
...
func BatchReStart(ctx context.Context, w rest.ResponseWriter, r *rest.Request) {
	...
	for _, info := range infos {
		...
		runErr := shandler.RestartDeviceFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if runErr != nil {
...
func BatchStartFromPxe(ctx context.Context, w rest.ResponseWriter, r *rest.Request) {
	...
	for _, info := range infos {
		...
		runErr := shandler.BootDeviceToPXEFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if runErr != nil {
...
func BatchPowerOff(ctx context.Context, w rest.ResponseWriter, r *rest.Request) {
	...
	for _, info := range infos {
		...
		runErr := shandler.PowerOffDeviceFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if runErr != nil {
...
func CheckPermissionForBatchOperate(ctx context.Context, w rest.ResponseWriter, r *rest.Request, infos []BatchOperateInfo) (bool, []BatchOperateInfo) {
	...
	for _, info := range infos {
		...
		sn, err := shandler.GetDeviceSnFromIpmitool(repo, logger, info.OobIp, info.Username, info.Password)
		if err != nil {
