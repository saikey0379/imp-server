...
func (repo *MySQLRepo) CountDeviceInstallCallbackByDeviceSNAndType(deviceSN string, callbackType string) (uint, error) {
	mod := model.DeviceInstallCallback{DeviceSN: deviceSN, CallbackType: callbackType}
	var count uint
	err := repo.db.Unscoped().Model(mod).Where("device_sn = ? and callback_type = ?", deviceSN, callbackType).Count(&count).Error
	return count, err
}
...
func (repo *MySQLRepo) GetDeviceInstallCallbackByDeviceSNAndType(deviceSN string, callbackType string) (*model.DeviceInstallCallback, error) {
	var mod model.DeviceInstallCallback
	err := repo.db.Unscoped().Where("device_sn = ? and callback_type = ?", deviceSN, callbackType).Find(&mod).Error
	return &mod, err
}
