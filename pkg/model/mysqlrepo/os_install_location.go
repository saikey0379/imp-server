...
func (repo *MySQLRepo) GetLocationListOrderByPId() ([]model.Location, error) {
	var mods []model.Location
	err := repo.db.Order("pid DESC").Find(&mods).Error
	return mods, err
}

func (repo *MySQLRepo) GetLocationIDList() ([]string, error) {
	var mods []model.Location
	var ids []string
...
func (repo *MySQLRepo) GetLocationIDListByPid(i string) ([]string, error) {
	var mods []model.Location
	var ids []string
	err := repo.db.Where("pid = ?", i).Order("name").Find(&mods).Error
	for _, v := range mods {
		ids = append(ids, strconv.Itoa(int(v.ID)))
	}
	return ids, err
...
func (repo *MySQLRepo) FormatLocationToTreeByPid(pid uint, content []map[string]interface{}, floor uint, selectPid uint) ([]map[string]interface{}, error) {
	...
	err := repo.db.Unscoped().Where("pid = ?", pid).Order("LPAD(name,255,0) ASC").Find(&mods).Error
	if err != nil {
		return content, err
	}
	//result[pid] = mods
...
func (repo *MySQLRepo) GetLocationNameById(id uint, content string, separator string) (string, error) {
	var mod model.Location
	if id <= uint(0) {
		return content, nil
	}
...
func (repo *MySQLRepo) GetLocationListByPid(pid uint) ([]model.Location, error) {
	var mods []model.Location
	err := repo.db.Where("pid = ?", pid).Find(&mods).Error
	return mods, err
}
