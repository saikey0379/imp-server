...
// device相关
func (repo *MySQLRepo) AddDevice(batchNumber string, sn string, hostname string, ip string, manageIp string, networkId uint, manageNetworkId uint, osId uint, hardwareId uint, systemId uint, locationId uint, locationU string, assetNumber string, status string, isSupportVm string, userID uint, devManager string, opsManager string, deviceLabel string, deviceDescribe string) (*model.Device, error) {
	mod := model.Device{BatchNumber: batchNumber, Sn: sn, Hostname: hostname, Ip: ip, ManageIp: manageIp, NetworkID: networkId, ManageNetworkID: manageNetworkId, OsID: osId, HardwareID: hardwareId, SystemID: systemId, LocationID: locationId, LocationU: locationU, AssetNumber: assetNumber, Status: status, IsSupportVm: isSupportVm, UserID: userID, DevManager: devManager, OpsManager: opsManager, DeviceLabel: deviceLabel, DeviceDescribe: deviceDescribe}
	err := repo.db.Create(&mod).Error
	return &mod, err
}

func (repo *MySQLRepo) UpdateDeviceById(id uint, batchNumber string, sn string, hostname string, ip string, manageIp string, networkId uint, manageNetworkId uint, osId uint, hardwareId uint, systemId uint, locationId uint, locationU string, assetNumber string, overDate string, status string, isSupportVm string, userID uint, devManager string, opsManager string, deviceLabel string, deviceDescribe string) (*model.Device, error) {
	mod := model.Device{BatchNumber: batchNumber, Sn: sn, Hostname: hostname, Ip: ip, ManageIp: manageIp, NetworkID: networkId, ManageNetworkID: manageNetworkId, OsID: osId, HardwareID: hardwareId, SystemID: systemId, LocationID: locationId, LocationU: locationU, AssetNumber: assetNumber, OverDate: overDate, Status: status, IsSupportVm: isSupportVm, UserID: userID, DevManager: devManager, OpsManager: opsManager, DeviceLabel: deviceLabel, DeviceDescribe: deviceDescribe}
	//设备信息发生修改，但属主不发生变化
	err := repo.db.Unscoped().First(&mod, id).Update("batch_number", batchNumber).Update("sn", sn).Update("hostname", hostname).Update("ip", ip).Update("manage_ip", manageIp).Update("network_id", networkId).Update("manage_network_id", manageNetworkId).Update("os_id", osId).Update("hardware_id", hardwareId).Update("system_id", systemId).Update("location_id", locationId).Update("location_u", locationU).Update("asset_number", assetNumber).Update("over_date", overDate).Update("status", status).Update("install_progress", 0.0000).Update("install_log", "").Update("is_support_vm", isSupportVm).Update("devManager", devManager).Update("opsManager", opsManager).Update("device_label", deviceLabel).Update("device_describe", deviceDescribe).Error
	return &mod, err
}
...
func (repo *MySQLRepo) OnlineDeviceById(id uint) (*model.Device, error) {
	mod := model.Device{}
	err := repo.db.Unscoped().First(&mod, id).Update("status", "online").Error
	return &mod, err
}

func (repo *MySQLRepo) OfflineDeviceById(id uint) (*model.Device, error) {
	mod := model.Device{}
	err := repo.db.Unscoped().First(&mod, id).Update("status", "offline").Error
	return &mod, err
}
...
func (repo *MySQLRepo) CreateBatchNumber() (string, error) {
	...
	if count > 0 {
		...
		fix := utils.SubString(device.BatchNumber, 8, len(device.BatchNumber)-8)
...
func (repo *MySQLRepo) CountDeviceByLocationId(id uint) (uint, error) {
	mod := model.Device{LocationID: id}
	var count uint
	err := repo.db.Unscoped().Model(mod).Where("location_id = ?", id).Count(&count).Error
	return count, err
}
...
func (repo *MySQLRepo) CountDevice(where string) (int, error) {
	row := repo.db.DB().QueryRow("SELECT count(t1.id) as count FROM devices t1 left join networks t2 on t1.network_id = t2.id left join os_configs t3 on t1.os_id = t3.id left join hardwares t4 on t1.hardware_id = t4.id left join system_configs t5 on t1.system_id = t5.id left join manufacturers t8 on t1.sn = t8.sn" + where)
	var count = -1
	if err := row.Scan(&count); err != nil {
		return 0, err
	}
...
func (repo *MySQLRepo) GetDeviceListWithPage(limit uint, offset uint, where string) ([]model.DeviceFull, error) {
	...
	sql := "SELECT t1.*,t2.network as network_name,t6.network as manage_network_name,t3.name as os_name,concat(t4.company,'-',t4.model_name) as hardware_name,t5.name as system_name,t7.username as owner_name,t8.ip as bootos_ip,t8.oob as oob_ip,t8.cpu_sum as cpu_sum,t8.memory_sum as memory_sum,t8.model_name as model_name,t8.is_vm as is_vm,t8.version_agt as version_agt,t8.gpu as gpu FROM devices t1 left join networks t2 on t1.network_id = t2.id left join os_configs t3 on t1.os_id = t3.id left join hardwares t4 on t1.hardware_id = t4.id left join system_configs t5 on t1.system_id = t5.id left join manage_networks t6 on t1.manage_network_id = t6.id left join `users` t7 on t1.user_id = t7.id left join manufacturers t8 on t1.`sn` = t8.`sn` " + where + " order by t1.hostname"
...
func (repo *MySQLRepo) GetFullDeviceBySn(sn string) (*model.DeviceFull, error) {
	var result model.DeviceFull
	err := repo.db.Raw("SELECT t1.*,t2.network as network_name,t6.network as manage_network_name,t3.name as os_name,concat(t4.company,'-',t4.model_name) as hardware_name,t5.name as system_name,t7.is_vm,t7.version_agt as version_agt FROM devices t1 left join networks t2 on t1.network_id = t2.id left join os_configs t3 on t1.os_id = t3.id left join hardwares t4 on t1.hardware_id = t4.id left join system_configs t5 on t1.system_id = t5.id left join manage_networks t6 on t1.manage_network_id = t6.id left join manufacturers t7 on t1.sn = t7.sn where t1.sn = ?", sn).Scan(&result).Error
	return &result, err
}

func (repo *MySQLRepo) GetFullDeviceById(id uint) (*model.DeviceFull, error) {
	var result model.DeviceFull
	err := repo.db.Raw("SELECT t1.*,t2.network as network_name,t6.network as manage_network_name,t3.name as os_name,concat(t4.company,'-',t4.model_name) as hardware_name,t5.name as system_name,t7.is_vm,t7.version_agt as version_agt FROM devices t1 left join networks t2 on t1.network_id = t2.id left join os_configs t3 on t1.os_id = t3.id left join hardwares t4 on t1.hardware_id = t4.id left join system_configs t5 on t1.system_id = t5.id left join manage_networks t6 on t1.manage_network_id = t6.id left join manufacturers t7 on t1.sn = t7.sn where t1.id = ?", id).Scan(&result).Error
	return &result, err
}
...
func (repo *MySQLRepo) GetDeviceByHostname(hostname string) (*model.Device, error) {
	var mod model.Device
	err := repo.db.Unscoped().Where("hostname = ?", hostname).Find(&mod).Error
	return &mod, err
}

func (repo *MySQLRepo) GetDeviceStatusBySn(sn string) (*model.Device, error) {
	var mod model.Device
	sql := "SELECT status FROM devices where sn = '" + sn + "'"
...
func (repo *MySQLRepo) GetDeviceBySnsAndNetworkId(sns []string, network_id uint) ([]*model.Device, error) {
	var mod []*model.Device
	err := repo.db.Unscoped().Where("sn in (?) and network_id = ?", sns, network_id).Find(&mod).Error
	return mod, err
}
